...
### 7. 🧲 智能全屏拖拽载入 (Smart Drag-and-Drop Loader)
*   **拖放即开**：支持全屏拖拽直接载入 `.xlsx` 或 `.xls` 格式的本地 Excel 文件。配有高保真森林绿毛玻璃悬浮背景（`backdrop-filter`）与动感微交互。
*   **拖拽句柄捕获**：在 Chrome / Edge 浏览器中拖入文件时，系统能自动通过拖拽事件捕获其原生的 `FileSystemFileHandle`，从而实现**拖拽载入后直接物理原位防抖同步保存**；在 Safari / Firefox 下则自动优雅降级为离线沙盒安全副本。
*   **物理句柄去重与联动**：系统会自动进行标签页去重检测。如果拖入的文件已经在现有 Tab 中打开，系统会自动高亮聚焦至对应标签页，避免重复打开产生冲突。
*   **0 闪烁防抖计数算法**：通过引入 React 级联拖拽层级计数器（`dragCounter`）与 CSS 事件穿透，完美消除了 HTML5 原生拖放 API 在经过子元素时高频闪烁的经典顽疾。

### 8. 📁 自由自主选定路径导出（智能“另存为”系统）
*   **系统级保存对话框**：在支持的浏览器下点击“导出”时，会自动唤起原生的系统保存对话框。用户可以自主选择将文件导出到**桌面、D盘、或任意工作目录**，并可直接更改名字。
*   **标签页自动同步升级**：如果用户当前处于“网页离线沙盒”只读副本状态，当他使用“导出”将其另存为到本地某处后，系统会**自动将该标签页升级为“💻 物理直写”状态**并绑定最新句柄。之后在此标签页的任何编辑，均会自动无感直接更新保存到该选定文件中。
